...
$active_sub_tab = 'journal doamin';
require_once('templates/header.php');
require_once('templates/sidebar.php');
if(!empty($_POST['user_action'])) {
  switch($_POST['user_action']) {
    case 'add':
        $data = $QueryFire->getAllData('domain',' id="'.trim(strip_tags($_POST['id'])).'"');
        if(empty($data)) {
          $QueryFire->insertData("domain",array('name'=>trim(strip_tags($_POST['name']))));
          $msg = 'Journal Domain added successfully.';
        } else {
          $msg = 'Journal Domain already exists.';
        }
        unset($data);
        break;
       case 'update':
        $data = $QueryFire->getAllData('domain',' name="'.trim(strip_tags($_POST['name'])).'" and id !='.$_POST['id']);
        if(empty($data)) {
          $QueryFire->upDateTable("domain",' id='.$_POST['id'],array('name'=>trim(strip_tags($_POST['name']))));
          $msg = 'Journal Domain updated successfully.';
        } else {
          $msg = 'Journal Domain already exists.';
        }
        unset($data);
        break;
    case 'delete':
        $QueryFire->upDateTable("domain",' id='.$_POST['id'],array('is_deleted'=>1));
        //$QueryFire->deleteDataFromTable("domain",' id='.$_POST['id']);
        $msg = 'Journal Domain deleted successfully.';
        break;
  }
}
$categories = $QueryFire->getAllData('domain',' is_deleted=0 order by id asc');
?>
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>Manage Journal Domain <button class="btn btn-primary dev-add">Add Journal Domain </button></h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="dashboard">Home</a></li>
            <li class="breadcrumb-item"><a href="products">Journal Management</a></li>
            <li class="breadcrumb-item active">Journal Domain</li>
          </ol>
        </div>
      </div>
    </div><!-- /.container-fluid -->
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="form-group">
              <label for="name">Journal Domain :</label>
              <input class="form-control category" name="name" required placeholder="Enter Journal Domain " type="text">
            </div>     
    ...
    $(document).ready(function() {
      ...
      jQuery(document).on("click",".dev-delete",function(e){
          if(jQuery(this).data("id") != "") {
              ...
              bootbox.confirm({
                  title: "Are you sure you want to delete this Journal Domain?",
